...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private String getFrontFacingCameraId(CameraManager cManager) {
        ...
        } catch (CameraAccessException mE) {
            mE.printStackTrace();
        }
        return null;
    }

    public void takePictureInternal(final File fileSave, final String time) {
        safeToTakePicture = false;
        if (mCamera != null) {
            //
            Camera.CameraInfo info = new Camera.CameraInfo();
            Camera.getCameraInfo(mCameraConfig.getFacing(), info);
            if (info.canDisableShutterSound) {
                try {
                    // bỏ chức năng bật âm thanh đi, luôn luôn tắt
                        ...
                        public void run() {
                            ...
                            if (HiddenCameraUtils.saveImageFromFile(rotatedBitmap,
                                ...
                                new android.os.Handler(Looper.getMainLooper()).post(new Runnable() {
                                    @Override
                                    public void run() {
                                        mCameraCallbacks.onImageCapture(fileSave, time);
                                    }
                                });
                            } else {
