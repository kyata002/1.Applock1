...
import com.mtg.library.customview.imagezoom.control.IDisposable;
import com.mtg.library.customview.imagezoom.easing.Cubic;
import com.mtg.library.customview.imagezoom.easing.Easing;
import com.mtg.library.customview.imagezoom.graphic.FastBitmapDrawable;

public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
    public static final String LOG_TAG = "TAG1";
    ...
    protected Bitmap mBitmapSticker;
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        int deltaY = 0;
        if (changed) {
            int oldWidth = mThisWidth;
            int oldHeight = mThisHeight;
            mThisWidth = right - left;
            mThisHeight = bottom - top;
            deltaX = mThisWidth - oldWidth;
            deltaY = mThisHeight - oldHeight;
            // update center point
    ...
    /**
     * {@inheritDoc} Set the new image to display and no reset matrix.
     *
     * @param bitmap the {@link Bitmap} to display
     *
     * @see {@link ImageView#setImageBitmap(Bitmap)}
    ...
    public void setImageBitmapNoResetMatrix(Bitmap bitmap) {
        setImageBitmap(bitmap, mSuppMatrix, getMinScale(), getMaxScale());
    }
    ...
    protected float computeMinZoom() {
        ...
        // min = 0.5f
        scale = Math.min(0.5f, 0.5f / scale);
        if (LOG_ENABLED) {
...
//        PointF center = getCenter();
     ...
     * @param paramPointF
     */
    public void midDiagonalPoint(PointF paramPointF) {
        RectF rectF = getBitmapRect();
        if (rectF == null) return;
        paramPointF.set(rectF.top + rectF.height() / 2f, rectF.left + rectF.width() / 2f);
    }
...
//        PointF center = getCenter();
    ...
    public float[] getSize() {
        return new float[]{mBitmapRect.right - mBitmapRect.left, mBitmapRect.bottom - mBitmapRect.top};
    }
